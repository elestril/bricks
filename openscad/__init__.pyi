# Stub package for the `openscad` module so editors/linters can resolve imports.
# Sourced from typings/openscad.pyi.
...
def find_face(obj:PyOpenSCAD,m:list[float] ) :
    """Find the face of the object which matches the given normal vector most
    m: vector of the normal
    """
    ...

def sitonto(obj:PyOpenSCAD,x:list[float],y:list[float],z:list[float] ):
    """
...
def debug(obj:PyOpenSCAD, faces:list[int]) -> None:
...
def add_menuitem(menuname:str, itemname:str, callback: str) -> None:
...
def marked(number:float) -> PyOpenSCAD:
